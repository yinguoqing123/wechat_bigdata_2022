...
def validate(model, val_dataloader):
    model.eval()
    mlm_losses, itm_losses, vm_losses, itc_losses = [], [], [], []
    with torch.no_grad():
        for batch in val_dataloader:
            mlm_loss, itm_loss, _, _, vm_loss, itc_loss = model(batch)
...
def pretrain(args):
    ...
    for epoch in range(args.max_epochs):
        for batch in train_dataloader:
            step += 1
            model.train()
            mlm_loss, itm_loss, mlm_accuracy, itm_accuracy, vm_loss, itc_loss = model(batch)
            ...
            itc_loss = itc_loss.mean() / accumulation_steps
